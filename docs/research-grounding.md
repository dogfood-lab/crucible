...
**Calibration — temperature scaling (added, §12 Q3).** The Q3 post-hoc step is now implemented: `metrics.fit_temperature` / `apply_temperature` / `temperature_scaled_ece` (Guo et al. 2017, arXiv:1706.04599) fit a single scalar `T` minimizing correctness-NLL and rescale the verdict-token-logprob confidence — lowering ECE without touching accuracy/agreement (it is monotonic in the confidence). The run report's `calibration` section reports per-judge `T` + ECE before/after. The fit is currently **in-sample** (the note says so loudly); a held-out / k-fold split is the honest next step. This sets up improving qwen's 0.121 ECE without re-running the panel — the next characterization run populates the section.
