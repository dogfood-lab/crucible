...
**Implication:** treat CoT-only detection as one signal among many in our ai-crucible; do not stake gate decisions on CoT readability alone.
...
**Implication:** environment-side hardening (lock the verifier, lock the test files, lock git history) is the highest-leverage single intervention — encode it into every ai-crucible puzzle by default.
...
**Implication:** detectors fed paired (hack, honest) trajectories outperform isolated classification by ~18 pp — give the ai-crucible's grader access to a known-honest reference trajectory whenever possible.
...
**Implication:** for closed-weight models we lose this signal — note that our ai-crucible's detection ceiling is strictly lower than what white-box labs can achieve; lean harder on environmental hardening and CoT-truncation probes to compensate.
...
**Implication:** reward hacking is not a benign symptom — generalizes to downstream misalignment. Our ai-crucible should treat any shortcut episode as a serious signal, not a minor scoring deduction.
...
## Cross-cutting takeaways for the ai-crucible's anti-shortcut hard gates
