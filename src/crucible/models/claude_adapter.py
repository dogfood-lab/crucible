...
Claude is the role crucible is built around: the :class:`~crucible.types.RoleName.DESIGNER`
stays Claude by design, and Claude is the default :class:`~crucible.types.RoleName.SOLVER`
...
exposes the judge surface so a :class:`~crucible.scoring.judge_panel.JudgePanel` can
drop it structurally when ``generator_family == "claude"``.

It mirrors :class:`~crucible.models.ollama_adapter.OllamaModel`'s public surface
...
:class:`~crucible.characterize.types.JudgmentRecord`'s metadata so a profile run is
...
class ClaudeModel:
    ...
    Public surface mirrors :class:`~crucible.models.ollama_adapter.OllamaModel` so the
    ...
    async def generate(self, state: AttemptState) -> str:
        ...
        Matches :data:`crucible.roles.GenerateFn` so a bound ``model.generate`` drops
    ...
    def as_judge(self) -> JudgeFn:
        ...
        :class:`~crucible.scoring.judge_panel.JudgePanel` reads ``.family`` off the
